                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <h4 class="card-title">Update Assign Pjp </h4><br><br>
                            @if (count($errors) > 0)
                            ...
                            @endif
                            <form method="post" class="row g-3" action="{{ route('assign_pjp.update', $assign_pjp->id) }}"
                                ...
                                <div class="col-4">
                                    ...
                                    <div class="form-group ">
                                        <input name="date" class="form-control" id="date_picker" type="date"
                                            value="{{ old('date', $assign_pjp->date) }}">
                                        @if ($errors->has('date'))
                                        ...
                                        <select name="employee_id" id="employee-dropdown" class="form-select select2">
                                            ...
                                            @foreach ($employees as $employee)
                                                <option value="{{ $employee->id }}"
                                                    {{ old('employee_id', $assign_pjp->employee_id) == $employee->id ? 'selected' : '' }}>
                                                    {{ $employee->name }}
                                                    @if ($employee->role_id)
                                                        ({{ $employee->role->name }})
                                                    @else
                                        ...
                                        <select name="beat_id[]" id="beat-dropdown" class="filter-multi-select" multiple>
                                            @foreach ($beats as $beat)
                                                <option value="{{ $beat->id }}"
                                                    {{ in_array($beat->id, explode(',', $beat_ids)) ? 'selected' : '' }}>
                                                    {{ $beat->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                ...
                                <div class="col-4">
                                    ...
                                    <div class="form-group col-sm-12">
                                        <select name="status" class="form-select" aria-label="Select status">
                                            <option value="1"
                                                {{ old('status', $assign_pjp->status) == '1' ? 'selected' : '' }}>Active
                                            </option>
                                            <option value="0"
                                                {{ old('status', $assign_pjp->status) == '0' ? 'selected' : '' }}>Inactive
                                            </option>
                                        </select>
                                    </div>
                                </div>
